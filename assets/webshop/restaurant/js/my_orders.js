      ...
      ${ordersData
        .map((order) => {
          ...
          return `<tr>
                    ...
                    <th class="order-status in-progress-status" >${orderStatus}</th>
  ...
  for (let i = 0; i < orderStatuses.length; i++) {
    if (saleStatus == orderStatuses[i]) {
      if (i == orderStatuses.length - 1) {
        ...
        document
          .getElementById(`${saleStatus}Circle`)
