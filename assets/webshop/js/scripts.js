...
// theme toggler functinality
let baseURL = document.getElementById("base_url").value;
let themesElement = document.getElementById("themes");
let defaultTheme = themesElement.value;
themesElement.addEventListener("change", async (e) => {
  let theme = ("value", e.target.value);
  localStorage.setItem("theme", JSON.stringify(theme));
  await callNewTheme(baseURL, theme);
});

async function callNewTheme(baseURL, theme) {
  const response = await fetch(baseURL + `webshop?theme=${theme}`, {
    method: "GET",
  });
