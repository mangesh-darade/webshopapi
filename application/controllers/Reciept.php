...
//print_r($ResEshopOrder); exit;
    ...
    /*     * *12-09-2020** */
    ///////////////////////////////// invoice_reciept/////////////////////////////////////////
    // public function invoice_reciept($id = null, $view = null, $save_bufffer = null) {
    //      $this->load->model('orders_model');
    //     $this->load->model('eshop_model');
        
    //     $order_hash_key = $this->input->post('order_id');
    //     if($order_hash_key){
    //         $order_details = $this->orders_model->getOrderByIDWithHashKey($order_hash_key);
    //         $id = $order_details->id;
    //     }
    //     // echo"<pre>";
    //     // print_r($id);
    ...
    public function invoice_reciept($id = null, $view = null, $save_bufffer = null) {
        ...
        //     $id = $order_details->id;
        // }     
        $order_details = $this->orders_model->getOrderByIDWithHashKey($id);
        $id = $order_details->id;
        $res = $this->eshop_model->validateRecieptEshopOrder($id);
        $_PID = $this->Settings->default_printer;
        $this->data['default_printer'] = $this->site->defaultPrinterOption($_PID);
    ...
    public function verify_whatsapp_mobile() {

        $code = $this->input->get('code', true);
        $phone = $this->input->get('phone', true);

        // Get country name by phone number
        $country_code_row = $this->db->select('country')
                                    ->where('phone', $phone)
                                    ->get('companies')
                                    ->row();

        if ($country_code_row && isset($country_code_row->country)) {
            $country_code = $this->getcountryCode($country_code_row->country);
            if ($country_code) {
                $full_phone = $country_code . $phone;
                if ($code && $phone) {
                    $this->load->model('Whatsapp_model');
                    $response = $this->Whatsapp_model->send_otp_by_whatsapp($full_phone, $code);
                    echo json_encode(['status' => 'success', 'response' => $response]);
                } else {
                    echo json_encode(['status' => 'error', 'message' => 'Missing phone or code']);
                }
            } else {
                echo json_encode(['status' => 'error', 'message' => 'Invalid country name: ' . $country_code_row->country]);
            }
        } else {
            echo json_encode(['status' => 'error', 'message' => 'No company found with this phone number']);
        }
    }

    public function getcountryCode($countryName) {
        $query = $this->db->select('code')
                        ->from('sma_country_master')
