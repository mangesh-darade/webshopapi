...
$symbol = isset($Settings) ? $Settings->symbol : '$';
$_wlSettings = isset($Settings) ? $Settings : null;
        ...
        <div class="wl-grid<?= $wl_count === 1 ? ' wl-grid--single' : '' ?>" id="wlGrid">
            ...
            <article class="wl-card<?= $unavailable ? ' wl-card--unavailable' : '' ?>" id="wishlist-item-<?= html_escape($card_key) ?>" data-product-id="<?= (int) $p_id ?>" data-variant-id="<?= (int) $list_vid ?>">
                ...
                <div class="wl-card-body">
                    ...
                    <p class="wl-card-variant"><?= htmlspecialchars($variant_label, ENT_QUOTES, 'UTF-8') ?></p>
                    <?php endif; ?>
                    ...
                    <div class="wl-card-price-row">
                        ...
                        <?php if ($mrp > $price && !$priceFromLabel): ?>
                        <span class="wl-card-mrp"><?= htmlspecialchars((string) $symbol, ENT_QUOTES, 'UTF-8') ?> <?= number_format($mrp, 2) ?></span>
                        <?php endif; ?>
                        <?php if ($discount >= 5 && !$priceFromLabel): ?>
                        <span class="wl-card-off"><?= (int) $discount ?>% OFF</span>
                        <?php endif; ?>
                    ...
                    <?php if ($statusLabel !== ''): ?>
                    <p class="wl-stock-status" role="status"><?= htmlspecialchars($statusLabel, ENT_QUOTES, 'UTF-8') ?></p>
                    <?php endif; ?>
                    <div class="wl-card-actions">
                        <?php if ($canPurchase): ?>
                        <button type="button" class="wl-btn wl-btn--primary"
                            data-wishlist-add="<?= (int) $p_id ?>"
                            data-variant-id="<?= (int) $list_vid ?>"
                            data-product-price="<?= htmlspecialchars((string) $price, ENT_QUOTES, 'UTF-8') ?>"
                            data-variant-price="<?= htmlspecialchars((string) $list_vprice, ENT_QUOTES, 'UTF-8') ?>"
                            data-variant-unit-quantity="<?= htmlspecialchars((string) $list_vuq, ENT_QUOTES, 'UTF-8') ?>">Add to cart</button>
                        <?php else: ?>
