...
class Webshop_model extends CI_Model {
    ...
    public function get_payment_gatways() {
        ...
        // Prefer dedicated gateways table (sma_payment_gateways with dbprefix).
        $q = $this->db->get('payment_gateways');

        if ($q->num_rows() > 0) {
            $row = (array) $q->row();
            if (array_intersect_key($gatewayDefaults, $row)) {
                return (object) array_merge($gatewayDefaults, $row);
            }
        }
        ...
        if ($q->num_rows() > 0) {
            return (object) array_merge($gatewayDefaults, (array) $q->row());
        }
    ...
    public function get_product_by_hash($productHash) {
        ...
        if ($q->num_rows() > 0) {
            ...
            $product['quantity'] = 0;
            $productStorageType = isset($data['item']['storage_type']) ? $data['item']['storage_type'] : '';
...
//       print_r($CustomerData);
  ...
  public function get_coupon_data($coupon_code) {
     ...
     * @return boolean
     */
    public function updatePaytmStatusOrder($id, $status, $note, $payStatus = '', $PaidAmt = 0) {
